 ...
 *
 *  Author: SmartThings - modified by jskarp5953
 *
 *  Date: 2013-04-30
 */
metadata {
    definition (name: "My SmartWeather Station Tile", namespace: "jskarp5953", author: "jskarp5953") {
        ...
        attribute "windGustVector", "string"
    ...
    tiles(scale: 2) {
        ...
        valueTile("windGust", "device.windGustVector", decoration: "flat", height: 1, width: 2) {
        ...
        details(["temperature", "feelsLike", "weatherIcon", "humidity", "wind", "windGust",
...
def pollUsingZipCode(String zipCode) {
    ...
    if (obs) {
        ...
        send(name: "windGust", value: obs.windGust as String, unit: windUnits) // as String because of bug in determining state change of 0 numbers
...
def pollUsingPwsId(String stationId) {
    ...
    if (obsWrapper && obsWrapper.observations && obsWrapper.observations.size()) {
        ...
        send(name: "windGust", value: convertWindGust(obs[dataScale].windGust, dataScale, tempUnits) as String, unit: windUnits) // as String because of bug in determining state change of 0 numbers
        send(name: "windGustVector", value: "${convertWindGust(obs[dataScale].windGust, dataScale, tempUnits)} ${windUnits}")
...
private convertWindGust(value, fromScale, toScale) {
    def fs = fixScale(fromScale)
    def ts = fixScale(toScale)
    if (fs == ts) {
        return value
